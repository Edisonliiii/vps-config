...
#######################################
function os_checker() {
  read_os_info="$(grep "^NAME" $"/etc/os-release" | awk -F'"' '{print $2}' | awk '{print $1}')"
  if [[ "$read_os_info" == "Ubuntu" || "$read_os_info" == "Debian" ]]; then
    # ubuntu
...
#######################################
# docker install on Centos (under 7)
# Globals:
# Arguments:
#   None
#######################################
...
function check_command_existence() {
  ...
  # [TODO] 这部分逻辑必须改掉！
  if [[ $command_name == "ufw" ]]; then
    command_bin_path="$1_BIN_PATH" # command_bin_path本身是一个变量名 想让其生效必须${!command_bin_path}
  else
    command_bin_path="NORMAL_BIN_PATH"
  fi

  if [ ! command -v $1 ] &>/dev/null || [ ! -f ${!command_bin_path}$1 ]; then # command doesn't exist
    #shift   # [Caution!] drop one arg
    echo "$1 could not be found"
    echo "Installing command $1..."
    shift
    # attemp by using system package manger first
    # if fail, call cooresponding installer
    installer+="${commandN_to_installN[$command_name]}"
    installer+="_"
    installer+="$OS_NAME"
    echo $installer
    if [[ $(type -t $installer) == function ]]; then # install via corresponding installer
      if ! $installer; then
        echo "[Status]: Installation crapped! Abort!"
        exit 0
      else
        echo "Installing using defined installer...."
        echo "[Status]: Successful!"
      fi
    else # install via system package manager
      if ! $PACKAGE_MANAGER install ${commandN_to_installN[$command_name]}; then
        echo "[Status]: $PACKAGE_MANAGER install error, alter to specific installer!"
      else
        echo "Installing using system PM...."
...
function run_blacklist_sqlite_container() {
  ...
  if [[ "$(docker images -q $BLACKLIST_IMAGE_NAME 2>/dev/null)" == "" ]]; then
...
# check_command_existence python3-pip
